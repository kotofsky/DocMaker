...
namespace DocMaker
{
    public class WordBuilder
    {
        ...
        public byte[] Build(string templatePath, DocTemplate template)
        {
            ...
            return Generate(templatePath, template.Tables.ToArray(), template.FieldsCollection);
        }

        private byte[] Generate(string templatePath, DocTable[] docTables, Dictionary<string, string> fields)
        {
            ...
            using (MemoryStream templateStream = new MemoryStream())
            {
                ...
                using (WordprocessingDocument doc = WordprocessingDocument.Open(templateStream, true))
                {
                    ...
                    // get WORD tables
                    Table[] originalTables = doc.MainDocumentPart.Document.Body.Elements<Table>().ToArray();
                    
                    for (int i = 0; i < docTables.Length; i++)
                    {
                        var clientTable = docTables[i];
